...
if ( ! class_exists( 'Greenpath_Rest_Api' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class Greenpath_Rest_Api {
		private static $instance;
		private $namespace;

		public function __construct() {
			// Init variables
			$this->set_namespace( 'Nuttergood/v1' );

			// Localize theme's main js script with rest variable
			add_filter( 'greenpath_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'greenpath_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}

		function register_rest_api_route() {
			$routes = apply_filters( 'greenpath_filter_rest_api_routes', array() );
	...
	Greenpath_Rest_Api::get_instance();
}
