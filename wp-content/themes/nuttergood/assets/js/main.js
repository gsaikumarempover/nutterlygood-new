(function ( $ ) {
	...
	$( document ).on(
		'greenpath_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder ) {
			var sliderOptions     = typeof $holder.data( 'options' ) !== 'undefined' ? $holder.data( 'options' ) : {},
				...
				autoHeight        = sliderOptions.autoHeight !== undefined && sliderOptions.autoHeight !== '' ? sliderOptions.autoHeight : false,
				...
				parallax          = $holder.hasClass( 'qodef-testimonials-list' ) && $holder.hasClass( 'qodef-col-num--1' ) && ($holder.hasClass( 'qodef-item-layout--info-below' ) || ($holder.hasClass( 'qodef-item-layout--info-boxed' ) && 'vertical' == direction) )? true : false;
			...
			if ( sliderOptions.slidesPerView1512 !== undefined && sliderOptions.slidesPerView1512 !== '' ) {
				slidesPerView1512 = 'auto' === sliderOptions.slidesPerView1512 ? 'auto' : parseFloat( sliderOptions.slidesPerView1512, 10 );
			}
			...
			if ( 'vertical' === direction ) {
				...
				slidesPerView1200 = slidesPerView;
				slidesPerView1024 = slidesPerView;
				slidesPerView880  = slidesPerView;
				spaceBetween      = 42;
				spaceBetween1512  = 42;
				spaceBetween1200  = 42;
				spaceBetween880   = 42;
				...
				on: {
					beforeInit: function () {
						if ( options.direction === 'vertical' ) {
							var height             = 0;
							var spacing            = 42; // default spacing between slides
							var maxHeight          = 220; // default height
							...
							if ( $item.length ) {
								$item.each(
									function () {
										...
										if ($thisHolder.innerHeight() > maxHeight) {
											maxHeight = $thisHolder.height();
										}

										maxHeight = maxHeight > $thisHolder.innerHeight() ? maxHeight : $thisHolder.innerHeight();
									}
								);
							}
							// adding 2x height to container since showing group of 2 items
							if ( qodefCore.windowWidth > 680 ) {
								$holder.css("height", maxHeight * 2 + spacing);
								$item.css("height", maxHeight * 2 + spacing);
							} else {
								$holder.css("height", maxHeight);
					...
					init: function () {
						...
						qodef.body.trigger(
							'greenpath_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodef404 = {
		init: function () {
			var $errorPage      = $( '#qodef-404-page' ),
				$headerHeight   = $( '#qodef-page-header' ).outerHeight(),
				$topAreaHeight  = $( '#qodef-top-area' ).outerHeight(),
				$height         = 0,
				$adminBarHeight = qodefGlobal.vars.adminBarHeight,
				$calcHeight     = '';
			...
			if ( $errorPage.length ) {
				...
				if ( qodefCore.body.hasClass( 'qodef-top-area--transparent' ) ) {
					$topAreaHeight = 0;
				}

				if ( qodef.windowWidth <= 1200 ) {
					$headerHeight  = $( '#qodef-page-mobile-header' ).outerHeight();
					$topAreaHeight = 0;

					if ( qodefCore.body.hasClass( 'qodef-mobile-header--transparent' ) ) {
						$headerHeight = 0;
					}
				}

				// Admin bar for mobile
				if ( qodef.windowWidth <= 782 && $adminBarHeight > 0 ) {
					$adminBarHeight = $adminBarHeight + 14;
				}

				if ( $topAreaHeight ) {
					$height = $headerHeight + $topAreaHeight + $adminBarHeight;
				} else {
					$height = $headerHeight + $adminBarHeight;
				}

				$calcHeight = 'calc(100vh - ' + parseInt( $height ) + 'px)';
	...
	var qodefAnchor = {
		...
		getHeaderHeight: function () {
			...
			if ( qodef.windowWidth > 1200 && qodefGlobal.vars.headerHeight !== null && qodefGlobal.vars.headerHeight !== '' ) {
...
(function ( $ ) {
	...
	var qodefMasonryLayout = {
		...
		setFixedImageProportionSize: function ( $masonry, $masonryItems, size, gap ) {
			var isMobileScreen 	  = qodef.windowWidth <= 880;
...
(function ($) {
	...
	var qodefWooButton = {
		init: function () {
			var $buttons = $( '.qodef-woo-single-inner .button:not(.compare):not(.yith-wcqv-button), ' +
				'.add_to_cart_button, ' +
				'.qodef-add-all-to-cart-button, ' +
				'.button.product_type_grouped, ' +
				'#yith-quick-view-content .single_add_to_cart_button ' +
				'.qodef-popup-compare .qodef-pli-add-to-cart button' );

			qodefWooButton.addHtml( $buttons );
		},
		addHtml: function ( $buttons ) {
			if ( $buttons.length ) {
				$buttons.each(
					function () {
						var $thisButton = $( this );
						if ( ! $thisButton.find( '.qodef-svg--button-icon' ).length ) {
							$thisButton.addClass( 'qodef-button qodef-layout--filled' );
							$thisButton.prepend(
								'<svg class="qodef-svg--button-icon" xmlns="http://www.w3.org/2000/svg" width="15.767" height="15.11" viewBox="0 0 15.767 15.11">' +
									'<g>' +
										'<g fill="#fff">' +
											'<path d="M14.217 7.469a.62.62 0 0 0-.705.52l-.58 4.041c-.066.46-.267 1.859-2.654 1.859H5.49c-2.387 0-2.588-1.4-2.654-1.859l-.58-4.041a.621.621 0 0 0-.705-.52.613.613 0 0 0-.532.69l.58 4.041c.27 1.877 1.651 2.911 3.891 2.911h4.788c2.24 0 3.621-1.034 3.891-2.911l.58-4.041a.613.613 0 0 0-.532-.69Z" />' +
											'<path d="M15.143 4.677h-1.875A5.247 5.247 0 0 0 8.015 0h-.262A5.247 5.247 0 0 0 2.5 4.677H.623a.611.611 0 1 0 0 1.222h14.52a.611.611 0 1 0 0-1.222Zm-7.39-3.455h.262a4.01 4.01 0 0 1 4 3.456h-8.26a4.01 4.01 0 0 1 3.998-3.456Z" />' +
										'</g>' +
									'</g>' +
									'<g fill="#fff">' +
										'<g>' +
											'<path d="M5.161 12.416a.611.611 0 0 1-.6-.51L4.14 9.381a.611.611 0 1 1 1.205-.2l.419 2.52a.611.611 0 0 1-.5.7.594.594 0 0 1-.103.015Z" />' +
											'<path d="M10.605 12.416a.62.62 0 0 1-.1-.008.611.611 0 0 1-.5-.7l.421-2.525a.611.611 0 1 1 1.205.2l-.424 2.518a.611.611 0 0 1-.602.515Z" />' +
										'</g>' +
										'<path d="M7.884 12.416a.611.611 0 0 1-.611-.611V9.281a.611.611 0 1 1 1.222 0v2.525a.611.611 0 0 1-.611.61Z" />' +
									'</g>' +
								'</svg>'
						);
							$thisButton.wrapInner( '<span class="qodef-m-text"></span>' );
						}
					}
				);
			}
				...
				$.ajax(
					{
						...
						success: function ( response ) {
							if ( response.status === 'success' ) {
								...
								qodefCore.shortcodes.greenpath_core_progress_bar.qodefProgressBar.init();
							} else {
								console.log( response.message );
							}
						},
