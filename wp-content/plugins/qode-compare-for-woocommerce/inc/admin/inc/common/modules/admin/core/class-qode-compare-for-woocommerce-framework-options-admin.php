...
class Qode_Compare_For_WooCommerce_Framework_Options_Admin extends Qode_Compare_For_WooCommerce_Framework_Options {
	private $menu_name;
	private $options_name;
	private $menu_label;
	private $icon_path;

	public function __construct( $menu_name = 'qode_compare_for_woocommerce_framework_menu', $options_name = 'qode_compare_for_woocommerce_framework_options', $params = array() ) {
		parent::__construct();

		$this->menu_name    = $menu_name;
		$this->options_name = $options_name;

		$this->menu_label = ! empty( $params['label'] ) ? $params['label'] : esc_html__( 'Qode Options', 'qode-compare-for-woocommerce' );
		$this->icon_path  = ! empty( $params['icon_path'] ) ? $params['icon_path'] : QODE_COMPARE_FOR_WOOCOMMERCE_ADMIN_URL_PATH . '/inc/common/modules/admin/assets/img/admin-logo-icon.png';
		...
		add_action( 'wp_ajax_qode_compare_for_woocommerce_action_framework_save_options_' . $this->get_options_name(), array( $this, 'save_options' ) );

		add_action( 'wp_ajax_qode_compare_for_woocommerce_action_framework_reset_options_' . $this->get_options_name(), array( $this, 'reset_options' ) );
	...
	public function framework_menu() {
		...
		do_action( 'qode_compare_for_woocommerce_action_framework_before_options_map', $this->get_menu_name() );

		$child_elements = $this->get_child_elements();

		if ( ! empty( $child_elements ) ) {
			add_submenu_page(
				QODE_COMPARE_FOR_WOOCOMMERCE_GENERAL_MENU_NAME,
				$this->get_menu_label(),
				$this->get_menu_label(),
				'edit_theme_options',
				$this->get_menu_name(),
		...
		remove_submenu_page( QODE_COMPARE_FOR_WOOCOMMERCE_GENERAL_MENU_NAME, QODE_COMPARE_FOR_WOOCOMMERCE_GENERAL_MENU_NAME );
		do_action( 'qode_compare_for_woocommerce_action_framework_after_options_map', $this->get_menu_name() );
	}
	...
	public function init_options() {
		...
		do_action( 'qode_compare_for_woocommerce_action_framework_before_options_init_' . $this->get_options_name(), $this->get_options_name() );
		...
		do_action( 'qode_compare_for_woocommerce_action_framework_after_options_init_' . $this->get_options_name(), $this->get_options_name() );
	}

	public function populate_options() {

		do_action( 'qode_compare_for_woocommerce_action_framework_before_options_populate', $this->get_options_name() );
		...
		do_action( 'qode_compare_for_woocommerce_action_framework_after_options_populate', $this->get_options_name() );
	}

	public function register_options() {

		do_action( 'qode_compare_for_woocommerce_action_framework_before_options_registered', $this->get_options_name() );

		register_setting( $this->get_menu_label(), $this->get_options_name() );

		do_action( 'qode_compare_for_woocommerce_action_framework_after_options_registered', $this->get_options_name() );
	}
	...
	public function save_options() {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			$new_global_options = qode_compare_for_woocommerce_framework_clean_global_options( qode_compare_for_woocommerce_framework_map_deep_sanitize( wp_unslash( $_REQUEST ) ), $saved_global_options );

			unset( $new_global_options['action'] );

			check_ajax_referer( 'qode_compare_for_woocommerce_framework_ajax_save_nonce', 'qode_compare_for_woocommerce_framework_ajax_save_nonce' );
			...
			if ( $options_name === $this->get_options_name() ) {
				...
				do_action( 'qode_compare_for_woocommerce_action_framework_before_framework_option_save', $saved_global_options, $new_global_options );

				$this->set_options( array_merge( $saved_global_options, $new_global_options ) );

				update_option( $this->get_options_name(), $this->get_options() );

				esc_html_e( 'Saved', 'qode-compare-for-woocommerce' );

				do_action( 'qode_compare_for_woocommerce_action_framework_after_framework_option_save' );

			} else {
				esc_html_e( 'Wrong options trigger', 'qode-compare-for-woocommerce' );
			}
	...
	public function reset_options() {
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			if ( $options_name === $this->get_options_name() ) {
				...
				esc_html_e( 'Options reset to default', 'qode-compare-for-woocommerce' );

				do_action( 'qode_compare_for_woocommerce_action_framework_after_framework_option_reset' );
	...
	public function render_options() {
		...
		qode_compare_for_woocommerce_framework_template_part( QODE_COMPARE_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/admin/templates/holder', $custom_holder, $params );
	}
	...
	public function render_navigation() {
		...
		qode_compare_for_woocommerce_framework_template_part( QODE_COMPARE_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/admin/templates/navigation', '', $params );
	}
	...
	public function render_content() {
		...
		qode_compare_for_woocommerce_framework_template_part( QODE_COMPARE_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/admin/templates/content', '', $params );
	}
	...
	public function render_custom_content() {
		...
		qode_compare_for_woocommerce_framework_template_part( QODE_COMPARE_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/admin/templates/content', 'custom', $params );
	}
	...
	public function modify_post_row_actions( $actions ) {
		if ( $this->is_allowed_admin_cpt_page() ) {
			...
			foreach ( $actions as $action => $link ) {
				$svg_icon = qode_compare_for_woocommerce_get_svg_icon( $action );

				// If there is a svg with said action, add it before closing tag.
				if ( ! empty( $svg_icon ) ) {
					$pattern                = '/(<\/a\s*>)/i';
					$replacement            = qode_compare_for_woocommerce_get_svg_icon( $action, 'qodef-table-action-btn' ) . '$1';
					$new_actions[ $action ] = preg_replace( $pattern, $replacement, $link );
				} elseif ( 'inline hide-if-no-js' === $action ) {
					$pattern                = '/(<\/button\s*>)/i';
					$replacement            = qode_compare_for_woocommerce_get_svg_icon( 'quick-edit', 'qodef-table-action-btn' ) . '$1';
					$new_actions[ $action ] = preg_replace( $pattern, $replacement, $link );
				} else {
	...
	public function is_allowed_admin_cpt_page() {
		...
		$pages      = apply_filters( 'qode_compare_for_woocommerce_filter_framework_nav_pages', array( 'post.php', 'post-new.php', 'edit.php', 'edit-tags.php', 'term.php' ) );
		$cpts       = apply_filters( 'qode_compare_for_woocommerce_filter_framework_nav_cpts', array() );
		$taxonomies = apply_filters( 'qode_compare_for_woocommerce_filter_framework_nav_taxonomies', array() );
