(function ( $ ) {
	...
	$( document ).on(
		...
		function ( event, widget ) {
			...
			if ( $widgetID && $widgetID.val().includes( 'compare' ) ) {
			...
			tabs.addClass( 'qodef--init' ).tabs(
				{
					activate: function () {
						...
						$( document.body ).trigger( 'qode_compare_for_woocommerce_trigger_tab_change' );
					}
				}
			);
		},
		...
		qodefAddNewRow: function ( holder, $mainHolder ) {
			...
			$addButton.off().on(
				...
				function ( e ) {
					...
					$( document ).trigger(
						'qode_compare_for_woocommerce_add_new_row_trigger',
	...
	var qodefAdminOptionsPanel = {
			...
			if ( item.length ) {
				...
				setTimeout(
					function () {
						...
						$( document.body ).on(
							'qode_compare_for_woocommerce_trigger_tab_change',
							function () {
								qodefFramework.qodefColorPicker.checkFieldPosition( $newPane );
							}
						);
		...
		setActivePanel: function () {
			var cookie 	  = this.getCookie( 'qodefCompareActiveTab' );
			var urlParams = new URLSearchParams( window.location.search );
			var post      = urlParams.get( 'post' );
			var postType  = urlParams.get( 'post_type' );
		...
		saveOptionsInit: function ( $adminPage ) {
			this.optionsForm = this.adminPage.find( '#qode_compare_for_woocommerce_framework_ajax_form' );
			...
			if ( this.optionsForm.length ) {
				...
				this.optionsForm.on(
					...
					function ( e ) {
						...
						var form          = $( this ),
							button_action = buttonPressed === 'qodef_save' ? 'qode_compare_for_woocommerce_action_framework_save_options_' : 'qode_compare_for_woocommerce_action_framework_reset_options_',
							ajaxData      = {
