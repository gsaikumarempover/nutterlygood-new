...
abstract class Qode_Compare_For_WooCommerce_Framework_Field_Type {
	...
	public function __construct( $params ) {
		...
		/* Generate value */
		$value = isset( $params['value'] ) ? $params['value'] : qode_compare_for_woocommerce_framework_get_option_value( $this->scope, $this->type, $this->name, $this->default_value );
		...
		if ( ! empty( $this->dependency ) ) {
			...
			$show     = array_key_exists( 'show', $this->dependency ) ? qode_compare_for_woocommerce_framework_return_dependency_options_array( $this->scope, $this->type, $this->dependency['show'], true, $repeater ) : array();
			$hide     = array_key_exists( 'hide', $this->dependency ) ? qode_compare_for_woocommerce_framework_return_dependency_options_array( $this->scope, $this->type, $this->dependency['hide'], false, $repeater ) : array();
			$relation = array_key_exists( 'relation', $this->dependency ) ? $this->dependency['relation'] : 'and';

			$class[]         = qode_compare_for_woocommerce_framework_return_dependency_classes( $show, $hide );
			$dependency_data = qode_compare_for_woocommerce_framework_return_dependency_data( $show, $hide, $relation );
		}
	...
	public function load_assets() {
		do_action( 'qode_compare_for_woocommerce_action_framework_field_type_load_assets', $this->field_type );
	}

	public function render() {
		?>
		<div class="<?php echo esc_attr( $this->params['class'] ); ?>" id="qodef_<?php echo esc_attr( $this->params['id'] ); ?>" <?php qode_compare_for_woocommerce_inline_attrs( $this->params['dependency_data'], true ); ?>>
					...
					<div class="qodef-field-desc">
						...
						<?php if ( '' !== $this->description ) { ?>
							<p class="qodef-description qodef-field-description">
								...
								echo qode_compare_for_woocommerce_framework_wp_kses_html( 'description', $this->description );
								?>
							</p>
						<?php } ?>
					</div>
