...
class Qode_Compare_For_WooCommerce_Framework_Field_WP_Image extends Qode_Compare_For_WooCommerce_Framework_Field_WP_Type {
	...
	public function render_field() {
		...
		<div class="qodef-image-uploader" data-file="no" data-multiple="<?php echo esc_attr( $this->multiple ); ?>">
			<div class="qodef-image-thumb <?php echo esc_attr( $hide_class ); ?>">
				<?php if ( 'yes' === $this->multiple ) { ?>
						...
						if ( '' !== $this->params['value'] ) {
							...
							foreach ( $images_array as $image_id ) :
								...
								if ( is_array( $image_src ) ) {
									echo '<li ><img src="' . esc_url( $image_src[0] ) . '" alt="' . esc_attr__( 'Image Thumbnail', 'qode-compare-for-woocommerce' ) . '" /></li>';
								}
							endforeach;
						}
						?>
					...
					if ( '' !== $this->params['value'] ) {
						...
						?>
						<img class="qodef-single-image" src="<?php echo esc_url( $image_src[0] ); ?>" alt="<?php esc_attr_e( 'Image Thumbnail', 'qode-compare-for-woocommerce' ); ?>"/>
						<?php
					}
				}
				?>
			...
			<a class="qodef-image-upload-btn" href="javascript:void(0)" data-frame-title="<?php esc_attr_e( 'Select Image', 'qode-compare-for-woocommerce' ); ?>" data-frame-button-text="<?php esc_attr_e( 'Select Image', 'qode-compare-for-woocommerce' ); ?>"><?php esc_html_e( 'Upload', 'qode-compare-for-woocommerce' ); ?></a>
