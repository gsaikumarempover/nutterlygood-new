...
abstract class Qode_Compare_For_WooCommerce_Framework_Field_WP_Type {
	public $type;
	public $field_type;
	public $name;
	...
	public function __construct( $params ) {
		...
		$value           = qode_compare_for_woocommerce_framework_get_option_value( '', $this->type, $this->name, $this->default_value );
		$params['value'] = $value;
		...
		if ( ! empty( $this->dependency ) ) {
			...
			$show     = array_key_exists( 'show', $this->dependency ) ? qode_compare_for_woocommerce_framework_return_dependency_options_array( '', $this->type, $this->dependency['show'], true ) : array();
			$hide     = array_key_exists( 'hide', $this->dependency ) ? qode_compare_for_woocommerce_framework_return_dependency_options_array( '', $this->type, $this->dependency['hide'] ) : array();
	...
	public function load_assets() {
		do_action( 'qode_compare_for_woocommerce_framework_action_field_wp_type_load_assets', $this->field_type );
	}

	public function render() {
		if ( 'div' === $this->params['layout'] ) { ?>
			<div class="<?php echo esc_attr( $this->params['row_class'] ); ?>" <?php qode_compare_for_woocommerce_inline_attrs( $this->params['dependency_data'], true ); ?>>
				...
				<div class="qodef-input-holder qodef-field-content">
					...
					echo qode_compare_for_woocommerce_framework_wp_kses_html( 'html', $this->render_field() );
					?>
				</div>
				<p class="description">
			...
			?>
			<tr class="<?php echo esc_attr( $this->params['row_class'] ); ?>" <?php qode_compare_for_woocommerce_inline_attrs( $this->params['dependency_data'], true ); ?>>
