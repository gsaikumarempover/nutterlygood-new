...
abstract class Qode_Compare_For_WooCommerce_Framework_Shortcode {
	...
	private function merge_option_atts( $params ) {
		...
		$this->option_atts                    = qode_compare_for_woocommerce_framework_map_shortcode_fields( $default_options, $params );
	}
	...
	public function get_editor_content( $content, $options ) {
		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_shortcode_content_html', $content, $options );
	}
	...
	public function parse_repeater_items( $items ) {
		...
		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_parse_repeater_items', $items_formatted );
	}

	public function init_holder_classes() {
		$holder_classes = array();

		$holder_classes[] = 'qcfw-shortcode';
		$holder_classes[] = 'qcfw-m';

		$atts             = $this->get_atts();
		$holder_classes[] = ! empty( $atts['custom_class'] ) ? esc_attr( $atts['custom_class'] ) : '';

		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_shortcode_holder_classes', $holder_classes );
	}
	...
	public function init_item_classes() {
		...
		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_shortcode_item_classes', $item_classes );
	}
	...
	public function import_shortcode_options( $params ) {
		...
		$options_to_skip = apply_filters( 'qode_compare_for_woocommerce_filter_framework_shortcode_options_import_skip', array( 'dependency' ) );
