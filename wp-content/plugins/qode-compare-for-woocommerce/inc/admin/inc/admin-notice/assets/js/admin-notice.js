(function ( $ ) {
	...
	var qodefDeactivationModal = {
		init: function () {
			this.deactivationLink  = $( '#the-list' ).find( '[data-slug="qode-compare-for-woocommerce"] span.deactivate a' );
			this.deactivationModal = $( '#qode-compare-for-woocommerce-deactivation-modal' );
		...
		initSubmitFunctionality: function () {
			var submitButton = this.deactivationModal.find( '.qodef-deactivation-modal-button-submit' ),
				...
				nonceHolder  = this.deactivationModal.find( '#qode-compare-for-woocommerce-deactivation-nonce' );
						...
						$.ajax(
							{
								type: 'POST',
								data: {
									action: 'qode_compare_for_woocommerce_deactivation',
									reason: reason,
