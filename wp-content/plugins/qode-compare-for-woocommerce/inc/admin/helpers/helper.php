...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_template_part' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_template_part( $root, $module, $template, $slug = '', $params = array() ) {
		$module_template_part = qode_compare_for_woocommerce_framework_get_template_part( $root, $module, $template, $slug, $params );
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_sanitize_module_template_part' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_sanitize_module_template_part( $template ) {
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_template_part' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_get_template_part( $root, $module, $template, $slug = '', $params = array() ) {
		$module   = qode_compare_for_woocommerce_framework_sanitize_module_template_part( $module );
		$template = qode_compare_for_woocommerce_framework_sanitize_module_template_part( $template );

		$temp = $root . '/' . $module . '/' . $template;

		$template = qode_compare_for_woocommerce_framework_get_template_with_slug( $temp, $slug );

		return qode_compare_for_woocommerce_framework_execute_template_with_params( $template, $params );
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_template_with_slug' ) ) {
	/**
	 * Loads module template part.
	 *
	 ...
	 * @return string - string with template path
	 */
	function qode_compare_for_woocommerce_framework_get_template_with_slug( $temp, $slug ) {
		$template = '';

		if ( ! empty( $temp ) ) {
			$slug = qode_compare_for_woocommerce_framework_sanitize_module_template_part( $slug );
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_execute_template_with_params' ) ) {
	/**
	 * Loads module template part.
	 *
	 ...
	 */
	function qode_compare_for_woocommerce_framework_execute_template_with_params( $template, $params ) {
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_svg_icon' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_svg_icon( $name, $class_name = '' ) {
		$svg_template_part = qode_compare_for_woocommerce_framework_get_svg_icon( $name, $class_name );

		echo wp_kses_post( $svg_template_part );
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_svg_icon' ) ) {
	/**
	 * Returns svg html
	 *
	 * @param string $name - icon name
	 ...
	 */
	function qode_compare_for_woocommerce_framework_get_svg_icon( $name, $class_name = '' ) {
		$html  = '';
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_wp_kses_html' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_wp_kses_html( $type, $content ) {
		switch ( $type ) {
			case 'description':
				$atts = array(
					'code' => apply_filters(
						'qode_compare_for_woocommerce_filter_framework_wp_kses_description_atts',
						array()
					),
				);
				break;
			case 'img':
				$atts = array(
					'img' => apply_filters(
						'qode_compare_for_woocommerce_filter_framework_wp_kses_img_atts',
						array(
							'itemprop' => true,
							'id'       => true,
							'class'    => true,
			...
			case 'svg':
				$atts = apply_filters(
					'qode_compare_for_woocommerce_filter_framework_wp_kses_svg_atts',
					array(
						'svg'      => array(
							'xmlns'             => true,
							'version'           => true,
			...
			case 'content':
				$atts = apply_filters(
					'qode_compare_for_woocommerce_filter_framework_wp_kses_content_atts',
					array(
						'div'  => array(
							'id'    => true,
							'class' => true,
				...
				break;
			default:
				return apply_filters( 'qode_compare_for_woocommerce_framework_filter_wp_kses_custom', $content, $type );
		}
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_extend_wp_kses_allowed_html' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_extend_wp_kses_allowed_html( $allowedposttags, $context ) {
	...
	add_filter( 'wp_kses_allowed_html', 'qode_compare_for_woocommerce_framework_extend_wp_kses_allowed_html', 10, 2 );
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_page_id' ) ) {
	/**
	 ...
	 * @return int
	 */
	function qode_compare_for_woocommerce_framework_get_page_id() {
		$page_id = get_queried_object_id();

		if ( qode_compare_for_woocommerce_framework_is_wp_template() ) {
			$page_id = - 1;
		}

		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_page_id', $page_id );
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_is_wp_template' ) ) {
	/**
	 * Function that checks if current page default wp page
	 *
	 * @return bool
	 */
	function qode_compare_for_woocommerce_framework_is_wp_template() {
		return is_archive() || is_search() || is_404() || ( is_front_page() && is_home() );
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_option_value' ) ) {
	/**
	 ...
	 * @return string|mixed
	 */
	function qode_compare_for_woocommerce_framework_get_option_value( $scope, $type, $name, $default_value = '', $post_id = null ) {
		...
		if ( 'admin' === $type ) {
			if ( ! empty( $scope ) ) {
				...
				$admin_options = qode_compare_for_woocommerce_framework_get_framework_root()->get_admin_option( $scope );
		...
		return apply_filters( 'qode_compare_for_woocommerce_filter_framework_get_option_value', $value, $name );
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_get_post_value_through_levels' ) ) {
	/**
	 ...
	 * @return string|array
	 */
	function qode_compare_for_woocommerce_framework_get_post_value_through_levels( $scope, $name, $post_id = 0 ) {
		$post_id = ! empty( $post_id ) ? intval( $post_id ) : qode_compare_for_woocommerce_framework_get_page_id();
		$value   = '';

		$option_value = qode_compare_for_woocommerce_framework_get_option_value( $scope, 'admin', $name );
		...
		if ( - 1 !== $post_id ) {
			$meta_value = qode_compare_for_woocommerce_framework_get_option_value( $scope, 'meta-box', $name, '', $post_id );
		...
		$value = apply_filters( 'qode_compare_for_woocommerce_filter_framework_value_through_levels_' . $name, $value );

		return $value;
	}
}

if ( ! function_exists( 'qode_compare_for_woocommerce_framework_map_shortcode_fields' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_map_shortcode_fields( $default_options, $params ) {
		$atts = (array) $params;
		$out  = array();
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_clean_global_options' ) ) {
	/**
	 ...
	 * @return array
	 */
	function qode_compare_for_woocommerce_framework_clean_global_options( $new_options, $saved_options ) {
...
if ( ! function_exists( 'qode_compare_for_woocommerce_framework_map_deep_sanitize' ) ) {
	/**
	 ...
	 */
	function qode_compare_for_woocommerce_framework_map_deep_sanitize( $value, $callback = 'sanitize_text_field' ) {
			...
			foreach ( $value as $index => $item ) {
				foreach ( $keys_array_sanitize as $key => $callback_function ) {
					if ( str_contains( $index, $key ) ) {
						$value[ $index ] = qode_compare_for_woocommerce_framework_map_deep_sanitize( $item, $callback_function );
						// if sanitized exit this each and continue first one.
						continue 2;
					}
				}
				$value[ $index ] = qode_compare_for_woocommerce_framework_map_deep_sanitize( $item );
			}
			...
			foreach ( $object_vars as $property_name => $property_value ) {
				foreach ( $keys_array_sanitize as $key => $callback ) {
					if ( str_contains( $property_name, $key ) ) {
						$value->$property_name = qode_compare_for_woocommerce_framework_map_deep_sanitize( $property_value, $callback );
						// if sanitized exit this each and continue first one.
						continue 2;
					}
				}
				$value->$property_name = qode_compare_for_woocommerce_framework_map_deep_sanitize( $property_value );
			}
		} else {
