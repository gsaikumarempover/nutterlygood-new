...
if ( ! class_exists( 'GreenPathMembershipTwitterApi' ) ) {
	/**
	 * Class GreenPathMembershipTwitterApi
	 */
	class GreenPathMembershipTwitterApi {
		/**
		 * @var $instance GreenPathMembershipTwitterApi class object
		 */
		...
		/**
		 * @return GreenPathMembershipTwitterApi
		 */
		 ...
		 *
		 * @see GreenPathMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'greenpath-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'greenpath-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'greenpath-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return greenpath_membership_get_membership_redirect_url();
		}
	}
}
