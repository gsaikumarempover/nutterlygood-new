...
if ( ! class_exists( 'GreenPathMembership_Page_Templates' ) ) {
	class GreenPathMembership_Page_Templates {
		...
		public function __construct() {
			...
			$this->set_templates(
				array(
					'user-dashboard.php' => esc_html__( 'Qode User Dashboard', 'greenpath-membership' ),
				)
			);
			...
			add_filter( 'greenpath_membership_filter_dashboard_template_name', array( $this, 'set_dashboard_template_name' ) );

			// Include dashboard page template content when user is logged in
			add_action( 'greenpath_membership_action_after_user_dashboard_page_content', array( $this, 'include_dashboard_template_content' ) );
		}

		/*
		 * @return GreenPathMembership_Page_Templates
		 */
		...
		public function get_template_path( $template ) {
			...
			if ( isset( $post ) && ! empty( $post ) && ! is_search() ) {
				...
				$file = GREENPATH_MEMBERSHIP_INC_PATH . '/general/page-templates/' . $page_template;
		...
		function include_dashboard_template_content() {
			...
			if ( is_user_logged_in() ) {
				greenpath_membership_template_part( 'general', 'page-templates/parts/content' );
			}
		}
	}

	GreenPathMembership_Page_Templates::get_instance();
}
