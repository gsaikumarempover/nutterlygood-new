		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
			...
			<li class="qodef-woo-side-area-cart-item qodef-e">
				...
				<div class="qodef-e-content">
					...
					<p class="qodef-e-price"><?php echo sprintf( esc_attr( $cart_item['quantity'] ) . esc_html__( ' x %s', 'greenpath-core' ), apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ) ); ?></p>
					<?php
					echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						...
						sprintf(
							...
							esc_attr__( 'Remove this item', 'greenpath-core' ),
							esc_attr( $product_id ),
							esc_attr( $cart_item_key ),
							esc_attr( $_product->get_sku() ),
							greenpath_core_get_svg_icon( 'close' )
						),
						$cart_item_key
					);
					?>
