...
if ( ! function_exists( 'greenpath_core_add_product_list_variation_catalogue' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_add_product_list_variation_catalogue( $variations ) {
		$variations['catalogue'] = esc_html__( 'Catalogue', 'greenpath-core' );

		return $variations;
	}

	add_filter( 'greenpath_core_filter_product_list_layouts', 'greenpath_core_add_product_list_variation_catalogue' );
}

if ( ! function_exists( 'greenpath_core_register_shop_list_catalogue_actions' ) ) {
	/**
	 * Function that override product item layout for current variation type
	 */
	function greenpath_core_register_shop_list_catalogue_actions() {
		...
		// Remove add to cart position on product list
		remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10 ); // permission 10 is default

		// Remove sale mark on product list
		remove_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_show_product_loop_sale_flash', 10 );

		// Remove price on product list
		remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_price', 10 ); // permission 10 is default
		remove_action( 'woocommerce_after_shop_loop_item_title', 'greenpath_add_price_per_unit_on_product', 11 ); // permission 5 is set because woocommerce_template_loop_product_title hook is added on 10

		// Remove countdown on product list
		remove_action( 'woocommerce_before_shop_loop_item', 'greenpath_core_woo_get_yith_countdown_on_list', 3 ); // permission 10 is default
	}

	add_action( 'greenpath_core_action_shop_list_item_layout_catalogue', 'greenpath_core_register_shop_list_catalogue_actions' );
}
