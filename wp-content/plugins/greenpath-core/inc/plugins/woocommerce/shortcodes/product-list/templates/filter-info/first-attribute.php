<?php if ( ! empty( $first_attribute_filter ) ) {
	...
	if ( ! empty( $params['product_ids'] ) ) {
		$attributes = wp_get_object_terms( $params['product_ids'], 'pa_' . $first_attribute_filter );
	} else {
		$attributes = get_terms( 'pa_' . $first_attribute_filter );
	}
	?>
			...
			<div class="qodef-e-options-inner">
				...
				foreach ( $attributes as $attribute ) {
							...
							array(
								...
								'tax_query'      =>
									array(
										...
										array(
											'taxonomy' => 'pa_' . $first_attribute_filter,
											'field'    => 'slug',
					...
					<?php if ( ! empty( $attribute_color ) ) { ?>
						<a href="#" class="qodef-color-holder qodef-e-link" data-id="<?php echo esc_attr( $attribute->slug ); ?>" data-type="<?php echo esc_attr( $first_attribute_filter ); ?>">
							<span class="qodef-e-color" style="background-color: <?php echo esc_attr( $attribute_color ); ?>"></span>
						</a>
					<?php } elseif ( ! empty( $attribute_label ) ) { ?>
						<a href="#" class="qodef-label-holder qodef-e-link" data-id="<?php echo esc_attr( $attribute->slug ); ?>" data-type="<?php echo esc_attr( $first_attribute_filter ); ?>">
							<span class="qodef-e-label"><?php echo esc_html( $attribute_label ); ?></span>
						</a>
					<?php } else { ?>
						<div class="qodef-e-checkbox">
							<input type="checkbox" id="<?php echo esc_attr( $id ); ?>" name="qodef-product-attribute" title="<?php echo esc_html( $attribute->name ); ?>" data-id="<?php echo esc_attr( $attribute->slug ); ?>" data-type="<?php echo esc_attr( $first_attribute_filter ); ?>" value="">
