...
if ( ! function_exists( 'greenpath_core_add_product_list_variation_horizontal' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_add_product_list_variation_horizontal( $variations ) {
		$variations['horizontal'] = esc_html__( 'Horizontal', 'greenpath-core' );

		return $variations;
	}

	add_filter( 'greenpath_core_filter_product_list_layouts', 'greenpath_core_add_product_list_variation_horizontal' );
}

if ( ! function_exists( 'greenpath_core_register_shop_list_horizontal_actions' ) ) {
	/**
	 * Function that override product item layout for current variation type
	 */
	function greenpath_core_register_shop_list_horizontal_actions() {
		...
		add_action( 'woocommerce_after_shop_loop_item_title', 'greenpath_add_short_description_on_product', 2 ); // permission 5 is set because woocommerce_template_loop_product_title hook is added on 10

		// Add additional tags around price
	...
	add_action( 'greenpath_core_action_shop_list_item_layout_horizontal', 'greenpath_core_register_shop_list_horizontal_actions' );
}
