...
if ( ! function_exists( 'greenpath_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'GreenPathCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'greenpath_core_filter_register_shortcodes', 'greenpath_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'GreenPathCore_Shortcode' ) ) {
	class GreenPathCore_Image_Marquee_Shortcode extends GreenPathCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'greenpath_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'greenpath_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( GREENPATH_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'greenpath_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'greenpath-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'greenpath-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'greenpath-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return greenpath_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
