...
if ( ! function_exists( 'greenpath_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'GreenPathCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'greenpath_core_filter_register_shortcodes', 'greenpath_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'GreenPathCore_Shortcode' ) ) {
	class GreenPathCore_Icon_Shortcode extends GreenPathCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( GREENPATH_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'greenpath_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'greenpath-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'greenpath-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'predefined_icon',
					'title'      => esc_html__( 'Predefined Icon', 'greenpath-core' ),
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'greenpath-core' ),
						'small'   => esc_html__( 'Small', 'greenpath-core' ),
						'medium'  => esc_html__( 'Medium', 'greenpath-core' ),
						'large'   => esc_html__( 'Large', 'greenpath-core' ),
						'huge'    => esc_html__( 'Huge', 'greenpath-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'greenpath-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'       => array(
						'normal' => esc_html__( 'Normal', 'greenpath-core' ),
						'circle' => esc_html__( 'Circle', 'greenpath-core' ),
						'square' => esc_html__( 'Square', 'greenpath-core' ),
					),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'greenpath-core' ),
					'group'      => esc_html__( 'Style', 'greenpath-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'hover_color',
					'title'      => esc_html__( 'Icon Hover Color', 'greenpath-core' ),
					'group'      => esc_html__( 'Style', 'greenpath-core' ),
					'dependency' => array(
						'hide' => array(
							'predefined_icon' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'greenpath-core' ),
					'group'      => esc_html__( 'Style', 'greenpath-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'greenpath_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return greenpath_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ( 'yes' === $atts['predefined_icon'] ) ? 'qodef--predefined-icon' : '';
		...
		private function get_styles( $atts ) {
			...
			if ( 'yes' === $atts['predefined_icon'] ) {

				if ( ! empty( $atts['custom_size'] ) ) {
					$styles[] = '--qode-width: ' . intval( $atts['custom_size'] ) . 'px';
				}

				if ( ! empty( $atts['color'] ) ) {
					$styles[] = '--qode-circle-color: ' . $atts['color'];
				}
			}
