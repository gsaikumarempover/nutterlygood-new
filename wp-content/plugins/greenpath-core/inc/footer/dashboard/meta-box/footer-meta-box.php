...
if ( ! function_exists( 'greenpath_core_add_page_footer_meta_box' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function greenpath_core_add_page_footer_meta_box( $page ) {

		if ( $page ) {
			$custom_sidebars = greenpath_core_get_custom_sidebars( true, true );
			$footer_columns  = apply_filters( 'greenpath_core_filter_footer_areas_columns_size', array() );

			$footer_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-footer',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Footer Settings', 'greenpath-core' ),
					'description' => esc_html__( 'Footer layout settings', 'greenpath-core' ),
				)
			);
				...
				for ( $i = 1; $i <= intval( $footer_columns['footer_top_sidebars_number'] ); $i ++ ) {
					$top_footer_area_section->add_field_element(
						array(
							...
							'title'       => sprintf( esc_html__( 'Custom Footer Top Area - Column %s', 'greenpath-core' ), $i ),
							'description' => sprintf( esc_html__( 'Widgets added here will appear in the %s column of top footer area', 'greenpath-core' ), $i ),
				...
				for ( $i = 1; $i <= intval( $footer_columns['footer_bottom_sidebars_number'] ); $i ++ ) {
					$bottom_footer_area_section->add_field_element(
						array(
							...
							'title'       => sprintf( esc_html__( 'Custom Footer Bottom Area - Column %s', 'greenpath-core' ), $i ),
							'description' => sprintf( esc_html__( 'Widgets added here will appear in the %s column of bottom footer area', 'greenpath-core' ), $i ),
							'options'     => $custom_sidebars,
						)
					);
				}
			...
			do_action( 'greenpath_core_action_after_page_footer_meta_box_map', $footer_tab );
		}
	}

	add_action( 'greenpath_core_action_after_general_meta_box_map', 'greenpath_core_add_page_footer_meta_box' );
}
