...
if ( ! function_exists( 'greenpath_core_weather_widget_logic' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_weather_widget_logic( $atts ) {
		$weather_data = array();
		...
		if ( ! $location ) {
			return greenpath_core_weather_widget_error();
		}
		...
		// set transient name
		$weather_transient_name = 'greenpath_core_' . $city_name_slug . '_' . $days_to_show . '_' . $units . '_' . $locale;
		...
		if ( get_transient( $weather_transient_name ) ) {
			...
			$weather_data['now'] = greenpath_core_weather_ping( $ping_params );

			if ( 5 == $days_to_show ) {
				$weather_data['forecast'] = greenpath_core_weather_ping( $ping_params, true );
			}

			if ( $weather_data['now'] || $weather_data['forecast'] ) {
				// set the transient, cache for three hours
				set_transient( $weather_transient_name, $weather_data, apply_filters( 'greenpath_core_filter_widget_weather_cache', 1800 ) );
			}
		}
...
if ( ! function_exists( 'greenpath_core_weather_ping' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_weather_ping( $params, $days = false ) {
		...
		if ( is_wp_error( $ping_get ) ) {
			return greenpath_core_weather_widget_error( $ping_get->get_error_message() );
		}
		...
		if ( isset( $data->cod ) and 404 == $data->cod ) {
			return greenpath_core_weather_widget_error( $data->message );
		} else {
			return $data;
		}
	}
...
if ( ! function_exists( 'greenpath_core_weather_widget_error' ) ) {
	/**
	 ...
	 * @return string
	 */
	function greenpath_core_weather_widget_error( $msg = false ) {

		if ( ! $msg ) {
			$msg = esc_html__( 'No weather information available', 'greenpath-core' );
		}
