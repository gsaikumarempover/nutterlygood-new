...
if ( ! function_exists( 'greenpath_core_add_page_title_meta_box' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function greenpath_core_add_page_title_meta_box( $page ) {
		...
		if ( $page ) {
			...
			$title_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-title',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Title Settings', 'greenpath-core' ),
					'description' => esc_html__( 'Title layout settings', 'greenpath-core' ),
				)
			);
			...
			$page_title_section->add_field_element(
				array(
					...
					'options'     => apply_filters( 'greenpath_core_filter_title_layout_options', array( '' => esc_html__( 'Default', 'greenpath-core' ) ) ),
				)
			);
			...
			do_action( 'greenpath_core_action_after_page_title_meta_box_map', $page_title_section );
		}
	}

	add_action( 'greenpath_core_action_after_general_meta_box_map', 'greenpath_core_add_page_title_meta_box' );
}

if ( ! function_exists( 'greenpath_core_add_general_page_title_meta_box_callback' ) ) {
	/**
	 ...
	 * @return array
	 */
	function greenpath_core_add_general_page_title_meta_box_callback( $callbacks ) {
		$callbacks['page-title'] = 'greenpath_core_add_page_title_meta_box';

		return $callbacks;
	}

	add_filter( 'greenpath_core_filter_general_meta_box_callbacks', 'greenpath_core_add_general_page_title_meta_box_callback' );
}
